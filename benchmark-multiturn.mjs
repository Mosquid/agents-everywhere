...
const clip = Buffer.concat([Buffer.alloc(24000), pcm, Buffer.alloc(48000 * 10)]);
const input = Buffer.concat([clip, Buffer.alloc((960 - clip.length % 960) % 960)]);
...
const directory = new URL(`./results/multiturn-${new Date().toISOString().replaceAll(':', '-')}/`, import.meta.url);
mkdirSync(directory, { recursive: true });
const results = [];
for (let trial = 1; trial <= 1; trial++) {
  let result = { trial, firstAudioPacketMs: null, firstNonSilentAudioMs: null,
    firstOutputTextMs: null, transcript: '', inputTranscript: '', delegations: 0,
    usage: null, maxSendLatenessMs: 0 };
  const newTurn = () => ({ trial: results.length + 1, firstAudioPacketMs: null,
    firstNonSilentAudioMs: null, firstOutputTextMs: null, transcript: '', inputTranscript: '',
    delegations: 0, maxSendLatenessMs: 0 });
  const output = [];
  const events = [];
  const opened = performance.now();
  const ws = new LiveWS(new OpenAI());
  let streamStart, stopped = false, finalized = false, sender;
  const deadline = setTimeout(() => ws.close(), 160000);
  try {
    for await (const item of ws) {
      ...
      if (event.type === 'session.started') {
        ...
        sender = (async () => {
          const sessionStart = streamStart;
          for (let offset = 0; offset < input.length * 10 && !stopped; offset += 960) {
            const target = sessionStart + offset / 48;
            await sleep(Math.max(0, target - performance.now()));
            if (stopped) break;
            if (offset > 0 && offset % input.length === 0) {
              results.push(result);
              console.log(result);
            ...
            const localOffset = offset % input.length;
            ws.send({ type: 'session.input_audio.append', audio: input.subarray(localOffset, localOffset + 960).toString('base64') });
          }
      ...
      if (event.type === 'session.output_audio.delta') {
        ...
        for (let i = 0; i + 1 < bytes.length; i += 2) {
          if (Math.abs(bytes.readInt16LE(i)) > 200) { result.firstNonSilentAudioMs ??= latency; result.lastNonSilentAudioMs = latency; break; }
        }
      }
...
const later = valid.filter(r => r.trial > 1).map(r => r.firstNonSilentAudioMs).sort((a,b) => a-b);
console.log({ laterTurnMedianMs: later[Math.floor(later.length / 2)], validTrials: valid.length, medianMs: sorted[Math.floor(sorted.length / 2)],
